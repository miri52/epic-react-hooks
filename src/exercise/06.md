...
? What does `useEffect` return? -> a cleanup function

? What is the purpose of an `ErrorBoundary` component? How can we create one?

? How can we reset the error boundary state?

-> We can clear the error and attempt to render the component tree again by
using the `resetErrorBoundary` function provided to the fallback component. For
example, we can call this function in response to a button click, allowing users
to retry a failed operation manually.

? Can you give examples of props that the `ErrorBoundary` component typically
accepts? What do we use them for?

-> `onError` is a function that gets called whenever an error is caught. This
function can be used to log errors to an error reporting service

`FallbackComponent` is a component that gets rendered whenever the
`ErrorBoundary` catches an error

`onReset` is a function that we use to cleanup/reset specific state in the app
before we re-render after an error

`resetKeys` is an array of values that when changed, will trigger a reset of the
error boundary. This is useful when changing certain props or state values
